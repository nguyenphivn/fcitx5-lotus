...
class LotusDBusHandler:
    ...
    def set_config(self, values_dict: dict) -> bool:
        ...
        Returns True only if the values actually reached Fcitx5, so callers can
        avoid reporting success when nothing was written.
        """
        if not self.iface:
            print("Failed to set config: not connected to Fcitx5")
            return False
        try:
    ...
    def set_sub_config_list(self, path: str, root_key: str, data_list: list) -> bool:
        ...
        Returns True only if the list actually reached Fcitx5.
        """
        if not self.iface:
            print(f"Failed to set sub config ({path}): not connected to Fcitx5")
            return False
        try:
            full_path = f"{self.addon_name}/{path}"
